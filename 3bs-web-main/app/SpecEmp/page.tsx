...
type Appointment = {
  ...
  service?: {
    serviceName: string;
    benchmark_time?: number; // ← add this
  };
    status: string;
  employee?: { id: string };
};
...
const calculateIdealTime = (count: number, appts: Appointment[]) => {
  const benchmarks: number[] = appts
    .map((appt) => appt.service?.benchmark_time)
  ...
  <div className="w-90">
    <Dropdown
      label="Avg Handling Time Chart :"
      options={["Bar Chart", "Radar Chart"]}
      selected={chartType}
      onSelect={(val) => setChartType(val as "Bar Chart" | "Radar Chart")}
    />
  </div>
</div>
...
/>
  ...
  <ResponsiveContainer width="100%" height={chartType === "Radar Chart" ? 250 : 250}>
    {chartType === "Bar Chart" ? (
      <BarChart data={responseTimeData}>
        <XAxis dataKey="date" tickFormatter={(dateStr: any) => {
    if (!dateStr) return "";
    ...
    return "";
        ...
        <Tooltip />
        <Bar dataKey="avgHandlingTime" fill="#4caf50" name="Actual Time" barSize={11} radius={[10, 10, 0, 0]} />
        <Bar dataKey="idealTime" fill="#8884d8" name="Ideal Time" barSize={11} radius={[10, 10, 0, 0]} />      </BarChart>
    ) : (
      <RadarChart outerRadius={90} data={responseTimeData}>
  <PolarGrid />
  <PolarAngleAxis dataKey="date" />
  <PolarRadiusAxis />
  <Radar className="" name="Actual Time" dataKey="avgHandlingTime" stroke="#4caf50" fill="#4caf50" fillOpacity={0.5} />
  <Radar name="Ideal Time" dataKey="idealTime" stroke="#8884d8" fill="#8884d8" fillOpacity={0.3} />
  <Tooltip />
  <Legend />
</RadarChart>
